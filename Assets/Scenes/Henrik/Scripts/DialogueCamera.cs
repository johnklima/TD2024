...
public class DialogueCamera : MonoBehaviour
{
    ...
    public GameObject diaTarget; // if null it will default to itself.

    [Header("Changes the speed which dialogue camera will lerp to position")] [Range(0, 5)]
    //I've put a "Range" modifier on it for the slider but if you reeeeally want to go beyond 5 you either change the max you change that here :)
    public float transitionSpeed = 1.0f; // 1 being the default speed currently!

    private bool bInDialogue;
    ...
    private void Start()
    {
        ...
        playerController =
            player.gameObject.GetComponent<PlayerController>(); // For Preventing movement during dialogue..

        if (diaTarget == null)
            diaTarget = transform
                .gameObject; // Default our camera focus to this object unless DiaTarget says otherwise (More flexibility).
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode
                .Space)) // Temporary methood to get the player out of dialogue mode, you can call this function however you'd like though!
    ...
    {
        if (bInDialogue)
            playerDiaCamOBJ.transform.LerpTowards(diaTarget.transform,
                transitionSpeed *
                Time.deltaTime); // Here we use the TranslateHelper in order to much cleanly lerp towards the pos and rot.
    }
}